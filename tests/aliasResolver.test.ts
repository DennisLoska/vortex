...
describe("resolveAssetAlias", () => {
  it("exact match returns canonical", () => {
    expect(resolveAssetAlias("faery/fix/hearts.png", ["faery/fix/hearts.png"]).alias).toBe(
      "faery/fix/hearts.png",
    );
  });
  it("wrong extension unique basename resolves", () => {
    const avail = ["faery/fix/hearts.png", "faery/fix/navi.gif"];
    expect(resolveAssetAlias("faery/fix/hearts.gif", avail).alias).toBe("faery/fix/hearts.png");
  });
  it("case insensitive", () => {
    expect(resolveAssetAlias("FAERY/FIX/HEARTS.PNG", ["faery/fix/hearts.png"]).alias).toBe(
      "faery/fix/hearts.png",
  ...
  it("missing extension resolves unique", () => {
    expect(resolveAssetAlias("faery/fix/hearts", ["faery/fix/hearts.png"]).alias).toBe(
      "faery/fix/hearts.png",
    );
  });
  it("fix/fixed confusion", () => {
    expect(resolveAssetAlias("faery/fixed/hearts.png", ["faery/fix/hearts.png"]).alias).toBe(
      "faery/fix/hearts.png",
    );
  });
  it("trims whitespace", () => {
...
describe("resolveFilterPreset", () => {
  it("case insensitive", () => {
    expect(resolveFilterPreset("grayscale", [...FILTER_PRESET_NAMES]).preset).toBe("Grayscale");
  });
  it("hyphen variant", () => {
    expect(resolveFilterPreset("gray-scale", [...FILTER_PRESET_NAMES]).preset).toBe("Grayscale");
  });
  it("grey alias", () => {
    ...
    expect(r.preset === "Grayscale" || r.suggestion.includes("Grayscale")).toBe(true);
  });
  it("unknown suggests", () => {
