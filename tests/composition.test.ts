...
function mockChild(label: string, x = 100, y = 100) {
  ...
  c.label = label;
  c.x = x; c.y = y;
  c.scale = { x: 0.5, y: 0.5, set: function(v: number){ this.x=v; this.y=v; } };
  c.removeFromParent = function(){ if(this.parent) this.parent.removeChild(this); };
  c.destroy = function(){};
  return c;
}
...
describe("CompositionAPI asset alias hardening", () => {
  it("removeAsset with wrong extension resolves to canonical", () => {
    ...
    webcam.visible = true;
    const textOverlay: any = { textPosition: null, currentIndex: 0, goTo(){}, next(){}, setTextPosition(){}, clear(){}, destroy(){}, };
  ...
  it("moveAsset with wrong extension resolves", () => {
    const bgLayer: any = { children: [] };
    const assetLayer = mockContainer();
    const fixedLayer = mockContainer([mockChild("faery/fix/navi.gif", 100, 100)]);
    const statusLayer: any = new Container();
    const webcam: any = new Container();
    const textOverlay: any = { textPosition: null, currentIndex: 0, goTo(){}, next(){}, setTextPosition(){}, clear(){} };
    const api = new CompositionAPI(bgLayer, assetLayer, fixedLayer, statusLayer, webcam, textOverlay, "faery");
  ...
  it("setBackground wrong extension resolves (mock)", async () => {
    const bgLayer: any = {
      children: [],
      setBackground: async (alias: string) => alias === "faery/backgrounds/bg.mp4",
    };
    const assetLayer = mockContainer();
